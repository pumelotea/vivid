<script setup lang="ts">
  ...
  const items = ref([
    ...
    },
    {
      ...
      action: (range: Range) => editorInstance.value.chain().focus().insertContentAt(range.to, "<p></p>").toggleBlockquote().run(),
    },
    {
      name: "标题1",
      cmd: "/h1",
      icon: "h-1",
      action: (range: Range) => editorInstance.value.chain().focus().insertContentAt(range.to, "<p></p>").setHeading({ level: 1 }).run(),
    },
    {
      name: "标题2",
      cmd: "/h2",
      icon: "h-2",
      action: (range: Range) => editorInstance.value.chain().insertContentAt(range.to, "<p></p>").setHeading({ level: 2 }).run(),
    },
    {
      name: "标题3",
      cmd: "/h3",
      icon: "h-3",
      action: (range: Range) => editorInstance.value.chain().insertContentAt(range.to, "<p></p>").setHeading({ level: 3 }).run(),
    },
    {
      name: "列表",
      cmd: "/list",
      icon: "list-unordered",
      action: (range: Range) => editorInstance.value.chain().focus().insertContentAt(range.to, "<p></p>").toggleBulletList().run(),
    },
    {
      name: "数学公式",
      cmd: "/math",
    ...
    },
    {
      ...
      cmd: "/code",
      icon: "brackets-line",
      action: (range: Range) => editorInstance.value.chain().focus().insertContentAt(range.to, "<p></p>").toggleCode().run(),
    },
    {
      name: "代码块",
      cmd: "/codeblock",
      icon: "code-view",
      action: (range: Range) => editorInstance.value.chain().focus().insertContentAt(range.to, "<p></p>").toggleCodeBlock().run(),
    },
  ]);
  const items2 = ref([
    {
    ...
    {
      name: "删除本行", icon: "close-line", action: (range: Range) => {
        const editor = editorInstance.value;
        const state = editorInstance.value.state;
        const tr = state.tr;
