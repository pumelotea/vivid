...
function getOverflowSide(element, container) {
  ...
  const isOverflowingLeft = Math.round(rect.left) < Math.round(containerRect.left);
  const isOverflowingRight = Math.round(rect.right) > Math.round(containerRect.right)
  return {
    isOverflowingLeft,
    isOverflowingRight
  }
...
function checkEqualityWithAdjacent(matrix, index, isColumn, isNext) {
  ...
  // 检查相邻索引是否越界
  if ((isColumn && (adjacentIndex < 0 || adjacentIndex >= columnCount)) ||
  ...
  for (let i = 0; i < (isColumn ? rowCount : columnCount); i++) {
    ...
    const adjacentElement = isColumn ? matrix[i][adjacentIndex] : matrix[adjacentIndex][i];
...
const observer = useMutationObserver(ViVidTable, (mutations) => {
...
<template>
  <node-view-wrapper class="vivid-table" @click="resetSelected()" ref="ViVidTable" :class="{ 'deleteCell' :isHover}">
    <div class="table-box" :class="{'table-over-left-shadow': isOverflowingLeft, 'table-over-right-shadow': isOverflowingRight}">
      ...
      <div class="table-controller-left">
        <div class="table-controller-bar-v">
          <template v-for="(h,index) in heights" :key="index">
            <div :style="`height:${h}px`" class="bar-button-v" :data-h="h" v-if="h > 0"
                 @click.stop="selectRow(index)">
              <control-button vertical :active="rowSelected === index || allSelected"
                              :show-delete-content="!allSelected"
                              :offset="index === 0 || index === widths.length -1 ? 0: 50"
                              :active-color="isHover?'rgba(255, 122, 139, 0.3)':'rgba(113,191,255,0.7)'">
                <template v-slot:tool="{isNext}">
                  <n-button type="info" text @click.stop="addRow(index, isNext?1:0)">
                    <i class="ri-add-line"></i>
                  </n-button>
                </template>
                <template #delete>
                  <n-button type="error" text @click.stop="deleteRow" @mouseover="handelDeleteHover()"
      ...
      >
        <template #trigger>
          <div class="table-controller-bar-lt"
               @click.stop="selectTable">
            <div class="bar-button-all" :class="{'table-all-cell-selected': allSelected}"></div>
          </div>
        </template>
        <n-button type="error" text @click="deleteTable" @mouseover="handelDeleteHover()"
                  @mouseleave="handelDeleteLeave()">
          <i class="ri-delete-bin-line"></i>
        </n-button>
      </n-popover>
        ...
        <div class="table-controller-top">
          <div class="table-controller-bar-h">
            <template v-for="(w,index) in widths" :key="index">
              <div :style="`width:${w}px`" class="bar-button-h" :data-w="w" v-if="w > 0"
                   @click.stop="selectColumn(index)">
                <control-button :active="colSelected === index || allSelected"
                                :show-delete-content="!allSelected"
                                :offset="index === 0 || index === widths.length -1 ? 0: 50"
                                :active-color="isHover?'rgba(255, 122, 139, 0.3)':'rgba(113,191,255,0.7)'">
                  <template v-slot:tool="{isNext}">
                    <n-button type="info" text @click="addCol(index, isNext?1:0)">
                      <i class="ri-add-line"></i>
                    </n-button>
                  </template>
                  <template #delete>
                    <n-button type="error" text @click.stop="deleteCol" @mouseover="handelDeleteHover()"
                              @mouseleave="handelDeleteLeave()">
                      <i class="ri-delete-bin-line"></i>
                    </n-button>
                  </template>
